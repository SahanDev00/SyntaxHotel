...
const TableBookings = () => {
  ...
  useEffect(() => {
    const fetchCustomers = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_BASE_URL}/booking/tablebookings`, {
          headers: {
          ...
          <tbody>
            {bookings.map((booking, index) => (
              ...
              >
                ...
                <td className="px-4 py-3">{booking.table_number}</td>
                ...
                <td className="px-4 py-3 flex justify-end gap-3">
                  <button className="text-blue-600 hover:text-blue-800">
                    <Link to={`/edit-table-booking/${booking.bookingID}`}>
                      <Pencil size={18} />
                    </Link>
                  </button>
                </td>
