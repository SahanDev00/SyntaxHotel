...
const EditTableBooking = () => {
  ...
  useEffect(() => {
    const fetchTableBooking = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_BASE_URL}/booking/tablebookings?BookingID=${bookingID}`, {
          headers: {
    ...
    useEffect(() => {
      const fetchCustomers = async () => {
        try {
          const response = await axios.get(`${process.env.REACT_APP_BASE_URL}/customers/customers?Status`, {
            headers: {
              APIkey: process.env.REACT_APP_APIKey
            }
          });
    ...
    useEffect(() => {
        const fetchTables = async () => {
          try {
            const response = await axios.get(`${process.env.REACT_APP_BASE_URL}/hotel/rooms?Status=Active`, {
              headers: {
                APIkey: process.env.REACT_APP_APIKey
              }
            });
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        if (response.data.error) {
          ...
          toast.success("Table booking Editted successfully!", {
            position: "top-right",
            autoClose: 2000,
          });
        }
      } catch (err) {
        toast.error("Failed to edit table booking");
        console.error(err);
      } finally {
        setLoading(false);
      }
  ...
  return (
    <div className="p-6 min-h-screen font-overpass bg-gray-50">
      ...
      <div className="flex justify-between items-center mb-6 pb-3 border-b">
        <h1 className="text-2xl font-bold text-gray-900">Edit Table Booking</h1>
      </div>
...
export default EditTableBooking;
