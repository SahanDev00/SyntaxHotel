...
const EditRoomTypes = () => {
  ...
  const formik = useFormik({
    initialValues: {
      roomTypeID: roomTypeID, // Always null for adding a new category
      type_name: roomType?.[0].type_name || "",
      description: roomType?.[0].description || "",
      price_per_night: roomType?.[0].price_per_night || "",
    },
    ...
    <div className="p-6 bg-gray-50 min-h-screen font-overpass">
      ...
      >
        ...
        <button
          ...
          disabled={loading}
        >
          {loading ? "Editing..." : "Edit Room Type"}
        </button>
      </form>
    </div>
  );
